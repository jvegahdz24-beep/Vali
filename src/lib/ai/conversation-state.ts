...
const log = logger.child({ module: 'conversation-state' })
...
async function loadFromDB(contactId: string): Promise<ConversationState | null> {
  try {
    ...
    log.warn({ contactId, err }, `[ConvState] Failed to load from DB for contact ${contactId}`)
    return null
  }
}
...
async function saveToDB(phone: string, contactId: string): Promise<void> {
  try {
    ...
    try {
      ...
      // FIX P1: If FK constraint fails (stale cached agent ID), invalidate cache and retry once
      const errMsg = upsertErr?.message || String(upsertErr)
      if (errMsg.includes('Foreign key') || errMsg.includes('FOREIGN KEY') || errMsg.includes('P2003')) {
        log.warn('[ConvState] FK error during upsert — invalidating agent ID cache and retrying')
        _cachedAgentId = null
        const freshAgentId = await getConversationAgentId()
        if (freshAgentId) {
          await db.agentMemory.upsert({
            where: {
              agentId_contactId_key: {
    ...
    log.warn({ contactId, err }, `[ConvState] Failed to save to DB for contact ${contactId}`)
  }
}
...
async function getConversationAgentId(): Promise<string | null> {
  ...
  try {
    ...
    log.warn({ err }, '[ConvState] Failed to lookup agent ID')
  }
  return null
}
...
export async function ensureStateLoaded(phone: string, contactId: string): Promise<void> {
  ...
  if (dbState) {
    ...
    log.info(`[ConvState] L2 cache hit for ${phone} — etapa: ${dbState.etapa}, nombre: ${dbState.nombre || 'N/A'}`)
  } else {
    ...
    for (const pregunta of preguntasEvitar) {
      ...
      for (const sentence of sentences) {
        ...
        if (sentenceNorm.includes(preguntaNorm) || 
          ...
          log.info(`[Middleware] Anti-repetición: eliminada pregunta sobre "${campo}"`)
          continue
        }
...
// FIX P0: Sanitize user-extracted values before injecting into system prompt
// to prevent prompt injection attacks.
// FIX CRITICAL-2: Unicode NFKC normalization + expanded pattern list + extraction-time sanitization
function sanitizeForPrompt(value: unknown, maxLen = 100): string {
  if (!value || typeof value !== 'string') return ''
  // Step 1: Unicode NFKC normalization — converts Cyrillic/Unicode lookalikes to Latin
  let cleaned = value.normalize('NFKC')
  // Step 2: Collapse whitespace (prevents "i g n o r e" bypass)
  cleaned = cleaned.replace(/\s+/g, ' ').trim()
  // Step 3: Truncate AFTER normalization
  cleaned = cleaned.slice(0, maxLen)
  // Step 4: Expanded injection pattern list (covers more attack vectors)
  const injectionPatterns = /(?:ignora(?:r)?(?:s|mos|n|ndo)?|olvida(?:r|do)?|forget(?:s|ting|ten)?|ignore(?:s|d)?|previous\s*instructions?|nuevas?\s*instrucciones?|you\s*are\s*now|act\s*as|pretend(?:s|ing|ed)?|roleplay(?:s|ing)?|system\s*prompt|override|jailbreak|hack(?:ing|ed|s)?|bypass|admin\s*mode|developer\s*mode|DAN\s*mode|evil\s*assistant|instruction(?:s)?\s*override|context\s*switch|new\s*task|disregard|no\s*longer|from\s*now\s*on|eres\s*un?\s*\w+|you\s*are\s*a\s*\w+|respond\s*(?:only|solo)\s*(?:with|con)|respond\s*as|output\s*only|return\s*only|stop\s*following|break\s*character|out\s*of\s*character|OOC)/gi
  cleaned = cleaned.replace(injectionPatterns, '[filtrado]')
  // Step 5: Remove template literals and code injection
  cleaned = cleaned.replace(/\{\{.*?\}\}/g, '[filtrado]')
  cleaned = cleaned.replace(/\$\{.*?\}/g, '[filtrado]')
  // Step 6: Strip all quotes to prevent breaking out of prompt strings
...
export function buildContextBlock(state: ConversationState): string {
  ...
  if (state.nombre) {
    parts.push(`• Nombre del contacto: "${sanitizeForPrompt(state.nombre)}"`)
  }
  if (state.tipo_negocio) {
    parts.push(`• Tipo de negocio: "${sanitizeForPrompt(state.tipo_negocio)}"`)
  }
  if (state.interes) {
    parts.push(`• Interés expresado: "${sanitizeForPrompt(state.interes)}"`)
  }
  ...
  if (state.dolor) {
    parts.push(`• Dolor/frustración principal: "${sanitizeForPrompt(state.dolor, 200)}"`)
  }
  if (state.producto) {
    parts.push(`• Producto mencionado: "${sanitizeForPrompt(state.producto)}"`)
  }
  if (state.presupuesto) {
    parts.push(`• Presupuesto: "${sanitizeForPrompt(state.presupuesto)}"`)
  }
  ...
  if (state.ultimaPregunta) {
    parts.push(`\n⚠️ ÚLTIMA PREGUNTA HECHA: "${sanitizeForPrompt(state.ultimaPregunta, 200)}" — NO repetirla.`)
  }

  if (state.preguntasHechas.length > 0) {
    parts.push(`⚠️ PREGUNTAS YA HECHAS: ${state.preguntasHechas.map(p => `"${sanitizeForPrompt(p, 150)}"`).join(', ')}`)
  }
...
export function detectarCorreccion(state: ConversationState, text: string): boolean {
  ...
  for (const pattern of correccionPatterns) {
    if (pattern.test(text)) {
      log.info(`[Middleware] Corrección detectada en: "${text.slice(0, 60)}"`)
      return true
    }
  }
