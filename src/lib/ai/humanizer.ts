...
const IDENTITY_REPLACEMENTS: [RegExp, string | ((match: string, ...args: any[]) => string)][] = [
  // Wrong name → Jhon (exact matches) — CATCH ALL POSSIBLE WRONG NAMES
  // The LLM may invent ANY name; catch the most common ones + generic patterns
  [/Me llamo Carlos\b/gi, 'Me llamo Jhon'],
  [/Soy Carlos\b/gi, 'Soy Jhon'],
  [/Mi nombre es Carlos\b/gi, 'Mi nombre es Jhon'],
  [/Me llamo Alex\b/gi, 'Me llamo Jhon'],
  [/Soy Alex\b/gi, 'Soy Jhon'],
  [/Mi nombre es Alex\b/gi, 'Mi nombre es Jhon'],
  ...
  // Catch "Carlos" or "Alex" used as self-identification anywhere
  [/Soy Carlos\b/g, 'Soy Jhon'],
  [/Soy Alex\b/g, 'Soy Jhon'],
  ...
  [/soy una IA/gi, ''],
  [/soy un bot\b/gi, ''],
  // Generic: "Soy [ANY_CAPITALIZED_WORD]" in self-intro → replace with Jhon
  // This catches ANY invented name the LLM might use
  [/^¡?Buenas\s+(?:noches|tardes|días)![\s.]*Soy\s+[A-Z][a-z]+/gm, (match) => {
    // Extract the name and replace with Jhon
    return match.replace(/Soy\s+[A-Z][a-z]+/, 'Soy Jhon')
  }],
  [/Soy [A-Z][a-z]+ del equipo de ValiAutoFlow/g, 'Soy Jhon del equipo de ValiAutoFlow'],
  [/Soy [A-Z][a-z]+ de ValiAutoFlow/g, 'Soy Jhon de ValiAutoFlow'],
...
export function enforceIdentity(text: string): string {
  ...
  // FIX: Also enforce identity in the SYSTEM PROMPT before LLM generation
  // This prevents the LLM from ever seeing a wrong name in its instructions
  // (called from revenue-engine.ts generateResponse)
