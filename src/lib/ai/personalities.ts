...
export function getSystemPrompt(
  ...
  // FIX: Added [EMPRESA] — JHON_SYSTEM_PROMPT uses [EMPRESA] but we only replaced [NOMBRE_AGENCIA]/[AGENCIA]
  const businessName = workspaceContext?.businessName || 'ValiFlow Pro'
  prompt = prompt.replace(/\[NOMBRE_AGENCIA\]/g, businessName)
  prompt = prompt.replace(/\[AGENCIA\]/g, businessName)
  prompt = prompt.replace(/\[EMPRESA\]/g, businessName)
