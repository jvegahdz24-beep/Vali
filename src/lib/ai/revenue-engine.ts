...
// 8-step pipeline: Analyze → Trigger → Decide → Objection →
// Generate → Follow-up → CRM → Route
// ═══════════════════════════════════════════════════════════════
...
const log = logger.child({ module: 'revenue-engine' })
...
export class RevenueEngine {
  ...
  private determineNextAction(
    ...
    // FIX: Improved close detection — if user asked about price AND didn't object,
    // and score is moderate-to-high, push toward closing
    const hasBudgetSignals = buySignals.some(s =>
      normalize(s).includes('cuanto') || normalize(s).includes('pago') ||
      normalize(s).includes('precio') || normalize(s).includes('cuota') ||
      normalize(s).includes('mensual') || normalize(s).includes('requisitos')
    )
    const hasNoPriceObjections = !objections.some(o => o.startsWith('precio'))

    // Strong buy signals → close immediately
    if (score > 70 && buySignals.length >= 2) return 'close'
    if (intent.intent === 'buy_signal' && intent.confidence > 0.5) return 'close'

    // NEW: User asked about price/requirements but didn't object → move to close
    // This captures the "cuánto sale?" → "3mil o 5mil??" flow
    if (hasBudgetSignals && hasNoPriceObjections && score >= 40) return 'close'

    // User asked about price and has objections → handle objection
    if (objections.length > 0) return 'handle_objection'

    // Moderate-to-high engagement → educate or close
  ...
  async generateResponse(
    ...
    try {
      log.info(`[RevenueEngine] generateResponse → calling chatWithAI (${messages.length} messages, provider: ${aiProvider}, last: "${(context?.lastMessage || '').slice(0, 50)}")`)
      ...
      log.info(`[RevenueEngine] generateResponse → got ${result.content.length} chars from ${result.model} (${result.latencyMs}ms)`)

      return this.parseJHONResponse(result.content, analysis)
    } catch (error) {
      const errMsg = error instanceof Error ? error.message : String(error)
      log.error({ err: errMsg }, '[RevenueEngine] LLM generation failed')
      log.error({ msgCount: messages.length, lastMsg: (context?.lastMessage || '').slice(0, 80) }, '[RevenueEngine] Failed messages count')
  ...
  async processConversation(params: {
    ...
    log.info(`[RevenueEngine] Lead analysis: score=${analysis.score}, stage=${analysis.stage}, temperature=${analysis.temperature}`)

    // Step 2: Detect triggers
    const trigger = this.detectTrigger(analysis)
    if (trigger.isActive) {
      log.info(`[RevenueEngine] Trigger active: ${trigger.triggerType} (${trigger.confidence})`)
    }

    // Step 3: Make decision
    const decision = this.makeDecision(analysis, trigger)
    log.info(`[RevenueEngine] Decision: ${decision.action} — ${decision.strategy.slice(0, 80)}`)
    ...
    try {
      ...
      log.error({ err: errMsg }, '[RevenueEngine] Response generation failed, using fallback')
      response = {
    ...
    log.info(`[RevenueEngine] Agent routing: ${agentRouting.agentType} (confidence: ${agentRouting.confidence})`)
