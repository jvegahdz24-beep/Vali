...
export class LeadProfiler {
  ...
  buildProfileContext(profile: LeadProfileData): string {
    ...
    try {
      painPoints = typeof profile.painPoints === 'string' ? JSON.parse(profile.painPoints) : profile.painPoints
    } catch { painPoints = [] }
  ...
  private toProfileData(profile: {
    ...
    let parsedPainPoints: string[] = []
    try { parsedPainPoints = JSON.parse(profile.painPoints || '[]') } catch { parsedPainPoints = [] }
    let parsedInterests: string[] = []
    try { parsedInterests = JSON.parse(profile.interests || '[]') } catch { parsedInterests = [] }
