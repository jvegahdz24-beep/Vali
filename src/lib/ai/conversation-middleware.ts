...
/** Tiempo de espera para agrupar mensajes (ms). 2s sigue agrupando mensajes
 *  escritos en ráfaga, pero recorta ~1.5s de latencia percibida vs 3.5s. */
...
export function postProcess(response: string, state: ConversationState): PostProcessResult {
  ...
  // 4. Red de seguridad PROPORCIONAL: si el filtrado se comió la mayor parte
  // del mensaje, quedan fragmentos rotos ("¡Mucho gusto, Pedro!)?", bullets
  // sueltos sin frase) que se ENVIABAN al cliente — visto en producción
  // 2026-07-11 (369→22, 256→31 chars). Mejor mandar el original completo
  // (peor caso: repite una pregunta) que mandar basura truncada.
  const origLen = original.trim().length
  const filtLen = filtered.trim().length
  if (origLen > 30 && (filtLen < 40 || filtLen < origLen * 0.4)) {
    console.log(`[Middleware] Post-proceso filtró demasiado (${origLen}→${filtLen}), usando original`)
    filtered = original
  }
