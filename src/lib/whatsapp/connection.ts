...
class WhatsAppManager {
  ...
  private async acquireProcessingLock(phone: string): Promise<boolean> {
    if (this._processingLocks.get(phone)) {
      log.info(`[WhatsApp] Processing lock held for ${maskPhone(phone)}, skipping`)
      return false
    }
  ...
  async start(): Promise<WhatsAppStatus> {
    ...
    if (this._connected && this.sock && !this.isSocketAlive()) {
      log.warn('[WhatsApp] Ghost detected on start() — forcing clean reconnect')
    ...
    this._connectBackground().catch((err) => {
      log.error({ err }, '[WhatsApp] Background connection failed')
      this._connecting = false
      this.emitStatus()
    })
  ...
  private async _connectBackground(): Promise<void> {
    try {
      ...
      this.sock.ev.on('creds.update', async () => {
        await saveCreds()
        if (dbAuth) {
          try { await dbAuth.save() } catch (e) { log.error({ err: e }, '[DB-AUTH] Save failed') }
        }
      })
      ...
      this.sock.ev.on('connection.update', async (update: any) => {
        ...
        log.info('[WhatsApp] STATUS:', connection || update)

        // QR code received — convert raw QR string to base64 PNG
        if (qr) {
          log.info('═══════════════════════════════════════════════')
          log.info('📱 ESCANEA ESTE QR CON WHATSAPP:')
          log.info('═══════════════════════════════════════════════')
          log.info(qr)
          log.info('═══════════════════════════════════════════════')
          try {
            ...
            log.info('[WhatsApp] QR code received and converted to PNG')
            this.emitQR(base64Png)
            this.emitStatus()
          } catch (err) {
            log.error({ err }, '[WhatsApp] Error converting QR to PNG')
        ...
        if (connection === 'open') {
          ...
          log.info('[WhatsApp] Connected! Phone:', this._phone)
          this.emitStatus()
        }
        ...
        if (connection === 'close') {
          ...
          if (statusCode === DisconnectReason.loggedOut) {
            log.info('[WhatsApp] Logged out, need new QR scan')
            this.qrData = null
            this._phone = null
            this._reconnectAttempts = 0
          } else if (statusCode === DisconnectReason.connectionClosed) {
            log.info('[WhatsApp] Connection closed, reconnecting...')
            this.scheduleReconnect(3000)
          } else if (statusCode === DisconnectReason.connectionLost) {
            log.info('[WhatsApp] Connection lost, reconnecting...')
            this.scheduleReconnect(2000)
          } else {
            log.info('[WhatsApp] Connection closed with code:', statusCode)
            if (statusCode !== DisconnectReason.loggedOut) {
      ...
      this.sock.ev.on('messages.upsert', async (m: any) => {
        try {
          ...
          log.info(`[WhatsApp] Message from ${maskPhone(phone)}: ${displayText.slice(0, 80)}...${mediaInfo ? ` [MEDIA: ${mediaInfo.type}]` : ''}`)
          this._lastActivity = new Date().toISOString()
          ...
          if (mediaInfo && this.sock) {
            downloadAndSaveMedia(this.sock, msg, mediaInfo)
              .then((id) => {
                if (id) log.info(`[WhatsApp] Media downloaded: ${id} (${mediaInfo.type})`)
              })
              .catch((err) => {
                log.warn({ err }, '[WhatsApp] Media download failed (non-critical)')
              })
          }
          ...
          if (text) {
            enqueueMessage(phone, text, pushName)
              .then((consolidatedText) => {
                this.processIncomingMessage(phone, consolidatedText, pushName!, remoteJid, msg.key.id!, { mediaInfo })
                  .catch((err) => {
                    log.error({ err }, '[WhatsApp] Error processing incoming message')
                  })
              })
              .catch((err) => {
                log.error({ err }, '[WhatsApp] Error in message batching')
                this.processIncomingMessage(phone, text, pushName!, remoteJid, msg.key.id!, { mediaInfo })
                  .catch((err2) => {
                    log.error({ err: err2 }, '[WhatsApp] Error processing fallback')
                  })
              })
          } else if (mediaInfo) {
            ...
            this.processIncomingMessage(phone, displayText, pushName!, remoteJid, msg.key.id!, { mediaInfo, isMediaOnly: true })
              .catch((err) => {
                log.error({ err }, '[WhatsApp] Error processing media-only message')
              })
          }
        } catch (err) {
          log.error({ err }, '[WhatsApp] Error processing messages.upsert event')
        }
      })

      log.info('[WhatsApp] Socket created, waiting for QR or connection...')
      ...
      this._connecting = false
      this._connected = false
      log.error({ err: error }, '[WhatsApp] Failed to start')
      this.emitStatus()
  ...
  async stop(): Promise<boolean> {
    try {
      ...
      try {
        ...
        log.warn('[WhatsApp] Could not clear auth directory')
      }

      ensureAuthDir()
      log.info('[WhatsApp] Disconnected and auth cleared')
      this.emitStatus()
      return true
    } catch (error) {
      log.error({ err: error }, '[WhatsApp] Error during stop')
      this._connected = false
      this._connecting = false
      this.sock = null
      this.emitStatus()
  ...
  async sendMessage(phone: string, text: string): Promise<{ success: boolean; id?: string; error?: string }> {
    ...
    try {
      ...
      this._lastActivity = new Date().toISOString()
      log.info(`[WhatsApp] Message sent to ${maskPhone(phone)}, id: ${result?.key?.id}`)
      ...
      log.error(`[WhatsApp] Failed to send message to ${maskPhone(phone)}:`, error)
      return {
        success: false,
        error: String(error),
      }
  ...
  async sendMessageRaw(jid: string, content: any): Promise<{ success: boolean; id?: string; error?: string }> {
    ...
    try {
      ...
      this._lastActivity = new Date().toISOString()
      log.info(`[WhatsApp] Raw message sent to ${jid}, id: ${result?.key?.id}`)
      return { success: true, id: result?.key?.id ?? undefined }
    } catch (error) {
      log.error(`[WhatsApp] Failed to send raw message:`, error)
  ...
  private async processIncomingMessage(
    ...
    try {
      ...
      log.info(`[WhatsApp] Reply generated for ${maskPhone(phone)} (${Date.now() - pipelineStart}ms, ${aiReplyText ? aiReplyText.length + ' chars' : 'NULL'})`)
      ...
      if (aiReplyText && this.sock && this._connected) {
        ...
        try {
          ...
          log.info(`[WhatsApp] Reply sent to ${maskPhone(phone)} (${finalText.length} chars, ${Date.now() - pipelineStart}ms total)`)
        } catch (sendErr) {
          log.error(`[WhatsApp] Send failed to ${maskPhone(phone)}:`, sendErr)
        }
      }
    } catch (error) {
      const errMsg = error instanceof Error ? error.message : String(error)
      log.error(`[WhatsApp] Pipeline failed for ${maskPhone(phone)} after ${Date.now() - pipelineStart}ms:`, errMsg)
    } finally {
...
const log = logger.child({ module: 'whatsapp-connection' })
...
if (globalForWhatsApp.whatsAppManager?.isSocketAlive()) {
  ...
  log.info('[WhatsApp] Reusing live instance from hot-reload')
} else {
  // Stale or dead instance — create fresh
  if (globalForWhatsApp.whatsAppManager) {
    log.info('[WhatsApp] Discarding stale instance (socket dead or not connected)')
  }
