...
export function Inbox({ workspaceId, onViewChange }: InboxProps) {
  ...
  const handleOpenTagDialog = () => {
    if (!currentConv?.contact?.id) {
      showNotif('Selecciona una conversación primero')
      return
    }
    const existingTags = currentConv.contact.tags || ''
    setTagInput(existingTags)
    setTagDialogOpen(true)
  }

  const handleSaveTags = async () => {
    if (!currentConv?.contact?.id) return
    try {
      const res = await fetch(`/api/contacts/${currentConv.contact.id}`, {
        method: 'PUT',
      ...
      if (res.ok) {
        ...
        setConversations(prev =>
          prev.map(c =>
            c.id === currentConv.id
              ? { ...c, contact: { ...c.contact, tags: tagInput } }
              : c
          )
        )
        setTagDialogOpen(false)
        showNotif('Etiquetas actualizadas')
      }
    } catch {
      showNotif('Error al guardar etiquetas')
    }
  }

  const handleRemoveTag = (tagToRemove: string) => {
    const currentTags = (currentConv?.contact?.tags || '').split(',').map(t => t.trim()).filter(Boolean)
    ...
    <div className="flex h-[calc(100vh-4rem)]">
      ...
      {tagDialogOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl border p-5 w-full max-w-md mx-4 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-semibold flex items-center gap-2">
                <Tag className="h-4 w-4 text-emerald-600" />
                Gestión de Etiquetas
              </h3>
              <button onClick={() => setTagDialogOpen(false)} className="h-6 w-6 rounded-full flex items-center justify-center hover:bg-muted text-muted-foreground">
                <X className="h-3.5 w-3.5" />
              </button>
            </div>
            <p className="text-xs text-muted-foreground mb-3">
              {currentConv?.contact?.firstName} {currentConv?.contact?.lastName}
            </p>
            ...
            {tagInput && (
              <div className="flex flex-wrap gap-1.5 mb-3">
                {tagInput.split(',').map(t => t.trim()).filter(Boolean).map(tag => (
                  <span key={tag} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 text-[11px] font-medium">
                    {tag}
            ...
            <div className="space-y-2">
              <Input
                ...
                className="text-sm"
              />
              <p className="text-[10px] text-muted-foreground">Ej: caliente, vip, follow-up, negociación</p>
            </div>

            {/* Actions */}
            <div className="flex gap-2 mt-4">
              <Button variant="outline" size="sm" className="flex-1" onClick={() => setTagDialogOpen(false)}>
                Cancelar
              </Button>
              <Button size="sm" className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white" onClick={handleSaveTags}>
                Guardar
              </Button>
            </div>
          </div>
