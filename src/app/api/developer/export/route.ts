  ...
  try {
    ...
    if (!workspaceId) {
      return NextResponse.json(
        { success: false, error: 'workspaceId es requerido' },
        { status: 400 }
      )
    }
    ...
    if (table === 'contacts') {
      records = await db.contact.findMany({ where: wsFilter, take: limit, orderBy: { createdAt: 'desc' } })
      total = await db.contact.count({ where: wsFilter })
    } else if (table === 'conversations') {
      records = await db.conversation.findMany({ where: wsFilter, take: limit, orderBy: { createdAt: 'desc' } })
      total = await db.conversation.count({ where: wsFilter })
      ...
      records = await db.deal.findMany({ where: wsFilter, take: limit, orderBy: { createdAt: 'desc' } })
      total = await db.deal.count({ where: wsFilter })
    } else if (table === 'agents') {
      records = await db.agent.findMany({ where: wsFilter, take: limit, orderBy: { createdAt: 'desc' } })
      total = await db.agent.count({ where: wsFilter })
    } else if (table === 'automations') {
      records = await db.automation.findMany({ where: wsFilter, take: limit, orderBy: { createdAt: 'desc' } })
      total = await db.automation.count({ where: wsFilter })
    }
