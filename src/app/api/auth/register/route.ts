...
export async function POST(req: NextRequest) {
  try {
    ...
    // Generate workspace slug from name. Evita colisión (dos empresas con el
    // mismo nombre chocaban en slug @unique → el 2º registro daba 500).
    const baseSlug = (name.trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')) || 'ws'
    let uniqueSlug = `${baseSlug}-workspace`
    const slugTaken = await db.workspace.findUnique({ where: { slug: uniqueSlug }, select: { id: true } }).catch(() => null)
    if (slugTaken) uniqueSlug = `${baseSlug}-${Date.now().toString(36).slice(-5)}`
